...
- 函数调用（Function Calling）：模型自主判断是否调用外部工具，代码执行后将结果回传；天气工具对接 Open-Meteo 真实开放接口（含城市名地理编码），实现"LLM 决策 + 工具执行"的最小智能体
...
- chat.py：主程序，对话循环、API 调用、流式输出与异常处
- agent.py：函数调用版智能体，模型决策调用天气工具，经两次请求完成问答，工具对接 Open-Meteo 真实天气 API
